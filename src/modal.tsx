...
const Modal: React.FC<IModalProps> = ({ show, close, title, icon, children, hideBg}) => {
	return (
		<div
			...
			<div
				...
				style={{ minHeight: 200, boxShadow: "0 -4px 10px -5px rgba(0, 0, 0, 0.1), 0 -8px 20px -5px rgba(0, 0, 0, 0.1)"}}>
			...
			<div className={"z-0 absolute top-0 left-0 w-full h-full bg-black " + (hideBg ? "opacity-0" : "opacity-50")} onClick={close}></div>
		</div>
	);
};
